      ...
      <div class="message-page-con message-category-con">
        <Menu width="auto" active-name="unread" @on-select="handleSelect">
          <MenuItem name="unread">
            <span class="category-title">未读消息</span><Badge style="margin-left: 10px" :count="aPerNoticeUnreadCount"></Badge>
          </MenuItem>
          <MenuItem name="readed">
            <span class="category-title">所有消息</span><Badge style="margin-left: 10px" class-name="gray-dadge" :count="aPerNoticeReadedCount"></Badge>
          </MenuItem>
        </Menu>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      perNoticeUnreadList: state => state.agent.perNoticeUnreadList,
      perNoticeReadedList: state => state.agent.perNoticeReadedList,
      noticeList () {
    ...
    ...mapGetters([
      'aPerNoticeUnreadCount',
      'aPerNoticeReadedCount'
    ])
  },
  methods: {
    ...mapMutations([
    ...
    ...mapActions([
      'agentgetContentByPerId',
      'agentgetUnreadPerList',
      'agentgetReadPerList'
    ]),
    handleSelect (name) {
    ...
    handleView (nid) {
      this.agentgetContentByPerId({ nid }).then(content => {
  ...
  mounted () {
    // 请求获取消息列表
    this.agentgetUnreadPerList()
    this.agentgetReadPerList()
  }
}
</script>
