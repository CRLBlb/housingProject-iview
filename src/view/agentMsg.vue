  ...
  <Card shadow>
    <div>
      <div class="message-page-con message-category-con">
        <Menu width="auto" active-name="unread" @on-select="handleSelect">
          <MenuItem name="unread">
            <span class="category-title">未读消息</span><Badge style="margin-left: 10px" :count="aMessageUnreadCount"></Badge>
          </MenuItem>
          <MenuItem name="readed">
            <span class="category-title">所有消息</span><Badge style="margin-left: 10px" class-name="gray-dadge" :count="aMessageReadedCount"></Badge>
          </MenuItem>
        </Menu>
      </div>
      <div class="message-page-con message-list-con">
        ...
        >
          <MenuItem v-for="item in messageList" :name="item.uid" :key="`msg_${item.uid}`">
            <div>
              <Avatar :src="item.uavatar" />{{ item.uname }}
            </div>
          </MenuItem>
        </Menu>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      messageUnreadList: state => state.agent.messageUnreadList,
      messageReadedList: state => state.agent.messageReadedList,
      messageList () {
  ...
  methods: {
    ...
    ...mapActions([
      'agentgetContentByUser',
      'agentgetUnreadMessageList',
      'agentgetReadMessageList',
      'agentreadMessage',
      'agentPostMessage'
    ]),
    handleSelect (name) {
    ...
    handleView (uid) {
      this.agentgetContentByUser({ uid }).then(content => {
        this.messageContent = content
        this.currentUid = uid
        this.agentreadMessage(uid)
        const item = this.messageList.find(item => item.uid === uid)
        if (item) this.showingMsgItem = item
      })
      this.agentgetUnreadMessageList()
      this.agentgetReadMessageList()
    },
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          this.agentPostMessage(post)
          this.agentgetUnreadMessageList()
          this.agentgetReadMessageList()
          this.handleView(this.currentUid)
